 ...
 */
  ...
  _updateItemsInRange(orderedItems, oldVisibleItems, range) {
    ...
    // Hide all previously visible items of hidden groups (except group is implicit group for global background items)
    ...
    for (let i = 0; i < orderedItems.byEnd.length; i++) {
      ...
      // @Sebu cache item duration?
      // @Sebu cache a maxDuration per group?
      // @Sebu different interval for items with end and without end?
