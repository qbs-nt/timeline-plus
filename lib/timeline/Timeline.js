    ...
    if (min != null && max != null) {
      ...
      util.forEach(this.itemSet.items, (item, key) => {
        if (item.groupShowing) {
          ...
          /* OLD: */ redrawQueueLength = redrawQueue[key].length;
          // NEW: if (redrawQueue[key].length > redrawQueueLength) {
          // NEW:   redrawQueueLength = redrawQueue[key].length;
          // NEW: }
        }
      })

      const needRedraw = redrawQueueLength > 0;
      // console.log('getItemRange 1', {redrawQueue, redrawQueueLength, needRedraw})
      if (needRedraw) {
